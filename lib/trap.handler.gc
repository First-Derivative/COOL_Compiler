...
#   - "_MemMgr_*" functions give an consistant view of the garbage
...
#
# Messages for the SncGC garabge collector
#

_SncGC_INITERROR:	.asciiz "Unable to initialize the garbage collector.\n"
_SncGC_HEAPEXP:		.asciiz "Expanding heap ...\n"
#_SncGC_HEAPEXP:		.asciiz ""
_SncGC_COLLECT:		.asciiz "Garbage collecting ...\n"
#_SncGC_COLLECT:		.asciiz ""
_SncGC_COLLERROR:	.asciiz "Garbage collection error.\n"
...
_instr_nonl:
	...
	andi	$gp $gp 0xfffffffc		# word align $gp
...
String.concat:
	...
	andi	$a0 $a0 0xFFFFFFFC		# align on word boundary
	...
	sub	$t1 $gp $a0			# bytes allocated by Object.copy
...
#        called before $s7 is exceded by $gp.
#
#     2) The initialization functions all take the same arguements as
#        defined in "_MemMgr_Init".
#
#     3) The garbage collector functions all take the arguements.  "$a0"
...
#
	...
	.globl _MemMgr_Alloc
_MemMgr_Alloc:
	add	$gp $gp $a0			# attemt to allocate storage
...
_MemMgr_QAlloc:
	add	$t0 $gp $a0			# attemt to allocate storage
...
#   This is a two generational scheme which uses an assignment table
...
#
#     1) Stack Pairs:
#        Every data item pushed onto the stack should be part of a
#        pair.  The garbage collector looks for "flags" to help
#        it determine if the following item on the stack is a pointer
#        or just data.  The following flags are valid:
#
#             0: Data (do not garbage collect)
#             1: Pointer (garbage collect)
#
#        The stack appears in memory as follows:
#
...
#        function.   Use "_GenGC_ChkCopy" to validate the pointer and
...
#
# SncGC Garbage Collector
#
#   This implements a "Stop-and-Copy" garbage collector for the COOL
#   runtime system.  As one area of storage fills up, a garbage
#   collection routine copies all "live" objects to the other area,
#   adjusts any pointers in the stack, registers, or objects, and
#   sets $gp (the allocation pointer) and $s7 (the limit pointer) to
#   their new values.  Both the stack and registers are searched to
#   find the "live" object roots.
#
#   The heap is setup as follows:
#
#      Header information for SncGC (size stored in "SncGC_INFOSIZE")
#       |
#       |   The left storage area (heap expansion can only occur here)
#       |    |
...
#     heap_start     $gp                $s7
#
#     $gp (allocation pointer): points to the next free word in the
#         current storage area.  It always moved from left to right
#         as pictured.
#
#     $s7 (limit pointer):  points to the word following the last word
...
#
#        These tests are performed whenever any data could be a pointer
#        to keep any non-pointers from being updated accidentally.
#
#     4) The size stored in the object does not include the word required
#        to store the eyecatcher for the object in the heap.  This allows
#        the prototype objects to not require its own eyecatcher.
#
...
SncGC_EXPANDSIZE=0x10000			# size to expand heap
SncGC_INFOSIZE=20				# size of SncGC header in heap (bytes)
...
# Initialization
#
#   The heap is first analyzed as the lower, middle and upper
#   limits of the storage areas are calculated.  The start of
#   the heap contains a header which stores information for
#   SncGC.  The rest of the heap is used for the storage areas.
#   Note that because the storage areas must be of the same
#   length, this function adjusts the heap limits accordingly
#   (i.e. makes sure the heap area less the size of the header
#   is even).  The header contains the limits of the storage
#   areas, the start of the stack, and the Register (REG) mask.
#
#   INPUT:
#	$a0: start of stack
#	$a1: initial Register mask
...
#
#   OUTPUT:
#	$gp: lower bound of the initial storage area
#	$s7: upper bound of the initial storage area
#
#   Registers modified
#	$t0,$t1,$gp,$s7
#
...
_SncGC_Init:
	la	$t0 heap_start			# set $t0 to the start of the heap
	addiu	$gp $t0 SncGC_INFOSIZE		# set $gp to the start of storage
	sub	$t1 $a2 $gp			# align storage so it is of even length
	blez	$t1 _SncGC_Init_error		# $a2 contained invalid address
	andi	$t1 $t1 7			# check for even length (number of words)
	beqz	$t1 _SncGC_Init_aligned
	addiu	$a2 $a2 -4			# align storage to even length
_SncGC_Init_aligned:
	bgt	$gp $a2 _SncGC_Init_error	# heap intially too small
	sw	$gp 0($t0)			# save the left limit of storage
	add	$t1 $gp $a2
	srl	$t1 $t1 1
	sw	$t1 4($t0)			# save the middle of storage
	move	$s7 $t1				# set $s7 to the limit of the area
	sw	$a2 8($t0)			# save the right limit of storage
	sw	$a0 12($t0)			# save the start of the stack
	li	$t1 SncGC_ARU_MASK		# apply Automatic Register Mask (ARU)
	and	$t1 $a1 $t1
	sw	$a1 16($t0)			# save the Register mask
	jr	$ra				# return
_SncGC_Init_error:
	la	$a0 _SncGC_INITERROR		# show error message
	li	$v0 4
	syscall
	li	$v0 10				# exit
	syscall
...
#
#   SncGC is a "stop and copy" garbage collector.  This function
#   verifies that the heap will be able to store a certain number of
#   words.  If not, a garbage collection or heap expansion is done
#   until the heap is large enough.  A heap expansion will occur only
#   under special circumstances:
#
#     1) The current storage area is #0.
#     2) The current storage area is 75% full
#
#   Once a heap expansion is done, the heap will continue to be
#   expanded and no more garbage collections will occur until
...
#
#   Registers modified:
#	$t0,$t1,$t2,$v0,$a0,$a1,$a2,$gp,$s7
#

	.globl	_SncGC_Collect
_SncGC_Collect:
	addiu	$sp $sp -12
	sw	$ra 12($sp)			# save return address
	sw	$a1 8($sp)			# save $a1, requested size
	sw	$a0 4($sp)			# non-zero = stack end, 0 = force expansion
_SncGC_Collect_loop:
	lw	$t0 4($sp)			# check if can only do expansion
	beq	$t0 $0 _SncGC_Collect_expand	# if zero, expand only
	la	$a0 _SncGC_COLLECT		# print collection message
	li	$v0 4
	syscall
	lw	$a0 4($sp)			# end of the stack to collect
	jal	_SncGC_StopCopy			# garbage collect
	bne	$a0 $0 _SncGC_Collect_continue	# wrong area, cannot expand storage
	sw	$0 4($sp)			# set flag to force expansion later
	jal	_SncGC_QLimits			# find the limits for the new area
	sub	$t0 $a1 $a0			# calculate 75% limit
	srl	$t0 $t0 2
	sub	$t0 $a1 $t0
	blt	$gp $t0 _SncGC_Collect_continue	# < 75% used, don't expand storage
_SncGC_Collect_expand:
	la	$a0 _SncGC_HEAPEXP		# print heap expansion message
	li	$v0 4
	syscall
	li	$v0 9				# sbrk
	li	$a0 SncGC_EXPANDSIZE		# set the size to expand the heap
	syscall
	li	$v0 9
	move	$a0 $zero
	syscall					# get new end of heap in $v0
	la	$a0 heap_start
	lw	$t0 0($a0)			# load the start of storage
	sub	$t0 $v0 $t0			# align storage so it is of even length
	andi	$t0 $t0 7			# check for even length (number of words)
	beqz	$t0 _SncGC_Collect_aligned
	addiu	$v0 $v0 -4			# align storage to even length
_SncGC_Collect_aligned:
	lw	$t0 0($a0)			# load the left limit of storage
	add	$t0 $t0 $v0
	srl	$t0 $t0 1
	sw	$t0 4($a0)			# save the middle of storage
	move	$s7 $t0				# set $s7 to the limit of the area
	sw	$v0 8($a0)			# save the right limit of storage
_SncGC_Collect_continue:
	lw	$a1 8($sp)			# load requested size into $a1
	add	$t0 $gp $a1			# test allocation
	bge	$t0 $s7 _SncGC_Collect_loop	# loop if still not enough
_SncGC_Collect_ok:
	lw	$ra 12($sp)			# restore return address
	addiu	$sp $sp 12
	jr	$ra				# return
...
#
#   To be conservative, all pointers are verified before an object
#   is moved to a different storage area.  This collection consists
#   of four phases:
#
#     1) The current storage area is switched.  This involves
#        finding the current area number, calculating the new
#        area, and saving the limits in $gp and $s7.
#
#     2) The stack is analyzed for root pointers into the heap.
#        The beginning of the stack is contained in the header
#        and the end is provided as input.  Two flags are
#        recognized.  If a 0 is found, the following item is
#        assumed not to be a pointer and is skipped.  If a 1 is
#        found, the following item is a pointer and is checked
#        (the flags are skipped in both cases).  If neither flag
#        is found, the current stack word is assumed to be a
#        pointer.  Pointers are checked to verify that they are
#        valid.  If so, they are copied and the stack word is
#        updated to reflect this.
#
#     3) If the bit #n in the Register mask is set in SncGC
#        header, the register #n is assumed to be a pointer.  It
#        is checked, and the object is copied if it has not
#        already been.  The register is then updated to refect
#        any movement in the object.  The initial Register mask
#        is set at initialization and can be changed at runtime
#        with a call to "_SncGC_SetRegMask"
#
#     4) At this point, all the root objects are in the new
#        storage area.  This area is now scanned object by object.
#        If an "Int" or "Bool" object is found, it is skipped.
#        If a "String" object is found, the string size pointer
#        (str_size) which points to an "int" object is checked and
#        copied.  All other objects' attributes are scanned and
#        each is treated as a pointer.  They are then checked and
#        copied into the new storage area.  Collection ends when
#        $gp is reached.  Note that because adding an object will
#        increase $gp, all objects reachable by the program will be
...
#
	...
	.globl	_SncGC_StopCopy
_SncGC_StopCopy:
	addiu	$sp $sp -24
	sw	$ra 24($sp)			# save return pointer
	sw	$a0 4($sp)			# save end of stack to collect
	jal	_SncGC_QArea			# get the current area
	sw	$a0 12($sp)			# save number of old area
	addi	$t0 $0 1			# calculate the new area
	sub	$a0 $t0 $a0
	sw	$a0 20($sp)			# save number of new area
	jal	_SncGC_QLimits			# get the limits of the area
	sw	$a0 16($sp)			# save lower limit of new area
	move	$gp $a0				# store the lower limit in $gp
	move	$s7 $a1				# store the upper limit in $s7
	la	$t0 heap_start			# start stack traversal
	lw	$t0 12($t0)			# get the start of the stack
	lw	$t1 4($sp)			# get the end of the stack
_SncGC_StopCopy_stackloop:
	ble	$t0 $t1 _SncGC_StopCopy_stkend	# finished
	lw	$a0 0($t0)			# get stack item
	bne	$a0 $0 _SncGC_StopCopy_not0	# check for 0 flag
	addiu	$t0 $t0 -8			# skip next item
	b _SncGC_StopCopy_stackloop		# loop
_SncGC_StopCopy_not0:
	addiu	$t1 $0 1			# check for 1 flag
	bne	$a0 $t1 _SncGC_StopCopy_error	# no flag found, error
	addiu	$t0 $t0 -4			# skip over flag, check next
	lw	$a0 0($t0)			# get stack item
_SncGC_StopCopy_not1:
	sw	$t0 8($sp)			# save index of stack loop
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	lw	$t0 8($sp)			# restore stack index
	sw	$a0 0($t0)			# save result into stack
	lw	$t1 4($sp)			# get the end of stack
	addiu	$t0 $t0 -4			# upodate stack index
	b	_SncGC_StopCopy_stackloop	# loop
_SncGC_StopCopy_stkend:
	la	$t0 heap_start
	lw	$t0 16($t0)			# get the Register mask
	sw	$t0 8($sp)			# save mask
_SncGC_StopCopy_reg16:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 16			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_reg17	# check if set
	move	$a0 $16				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$16 $a0				# update register
_SncGC_StopCopy_reg17:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 17			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_reg18	# check if set
	move	$a0 $17				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$17 $a0				# update register
_SncGC_StopCopy_reg18:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 18			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_reg19	# check if set
	move	$a0 $18				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$18 $a0				# update register
_SncGC_StopCopy_reg19:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 19			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_reg20	# check if set
	move	$a0 $19				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$19 $a0				# update register
_SncGC_StopCopy_reg20:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 20			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_reg21	# check if set
	move	$a0 $20				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$20 $a0				# update register
_SncGC_StopCopy_reg21:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 21			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_reg22	# check if set
	move	$a0 $21				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$21 $a0				# update register
_SncGC_StopCopy_reg22:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 22			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_reg24	# check if set
	move	$a0 $22				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$22 $a0				# update register
_SncGC_StopCopy_reg24:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 24			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_reg25	# check if set
	move	$a0 $24				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$24 $a0				# update register
_SncGC_StopCopy_reg25:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 25			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_reg30	# check if set
	move	$a0 $25				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$25 $a0				# update register
_SncGC_StopCopy_reg30:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 30			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_reg31	# check if set
	move	$a0 $30				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$30 $a0				# update register
_SncGC_StopCopy_reg31:
	lw	$t0 8($sp)			# restore mask
	srl	$t0 $t0 31			# shift to proper bit
	andi	$t1 $t0 1
	beq	$t1 $0 _SncGC_StopCopy_regend	# check if set
	move	$a0 $31				# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	move	$31 $a0				# update register
_SncGC_StopCopy_regend:
	lw	$t0 16($sp)			# get the start of new area
_SncGC_StopCopy_heaploop:
	bge	$t0 $gp _SncGC_StopCopy_heapend	# finished
	addiu	$t0 $t0 4			# skip over eyecatcher
	addiu	$t1 $0 -1			# check for eyecatcher
	lw	$t2 obj_eyecatch($t0)
	bne	$t1 $t2 _SncGC_StopCopy_error	# eyecatcher not found
	lw	$t1 obj_tag($t0)		# get the object's tag
	lw	$t2 _int_tag			# test for int object
	beq	$t1 $t2 _SncGC_StopCopy_int
	lw	$t2 _bool_tag			# test for bool object
	beq	$t1 $t2 _SncGC_StopCopy_bool
	lw	$t2 _string_tag			# test for string object
	beq	$t1 $t2 _SncGC_StopCopy_string
_SncGC_StopCopy_other:
	sw	$t0 8($sp)			# save pointer to object
	addi	$t1 $t0 obj_attr		# start at first attribute
_SncGC_StopCopy_objloop:
	lw	$t2 obj_size($t0)		# check if at end of object
	sll	$t2 $t2 2
	add	$t2 $t0 $t2
	bge	$t1 $t2 _SncGC_StopCopy_nextobj	# finished, do next object
	sw	$t1 4($sp)			# save the index into object
	lw	$a0 0($t1)			# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	jal	_SncGC_ChkCopy			# check and copy
	lw	$t1 4($sp)			# restore the index into object
	sw	$a0 0($t1)			# update object pointer
	lw	$t0 8($sp)			# restore pointer to object
	addiu	$t1 $t1 4
	b	_SncGC_StopCopy_objloop
_SncGC_StopCopy_string:
	sw	$t0 8($sp)			# save pointer to object
	lw	$a0 str_size($t0)		# set test pointer
	lw	$a1 12($sp)			# set old area number
	lw	$a2 20($sp)			# set new area number
	...
	sw	$a0 str_size($t0)		# update size pointer
_SncGC_StopCopy_int:
_SncGC_StopCopy_bool:
_SncGC_StopCopy_nextobj:
	lw	$t2 obj_size($t0)		# compute start of next object
	sll	$t2 $t2 2
	add	$t0 $t0 $t2
	b	_SncGC_StopCopy_heaploop	# loop
_SncGC_StopCopy_heapend:
	lw	$ra 24($sp)			# restore return pointer
	lw	$a0 20($sp)			# restore number of new area
	addiu	$sp $sp 24
	jr	$ra				# return
_SncGC_StopCopy_error:
	la	$a0 _SncGC_COLLERROR		# show error message
	li	$v0 4
	syscall
	li	$v0 10				# exit
	syscall
...
#   If it does, it then checks if a forwarding pointer was left.
#   If a forwarding pointer was left, it returns that; otherwise,
#   it copies the object to the $gp pointer and returns
#   the new address.  If the input pointer is not a heap object,
#   it does not modify the input register.  The forwarding pointer
#   is left in the object's dispatch table pointer.
#
#   INPUT:
#	$a0: pointer to check and copy
#	$a1: storage area in which the object should reside
#	$a2: storage area into which the object should be moved
#	$gp: current allocation pointer
#
#   OUTPUT:
#	$a0: if input points to an object then it is set to the
#            new location of object.  If not, it is unchanged.
#
#   Registers modified:
#	$t0,$t1,$t2,$v0,$a0,$a1,$gp
#

	.globl	_SncGC_ChkCopy
_SncGC_ChkCopy:
	addi	$sp $sp -12
	sw	$ra 12($sp)			# save the return address
	sw	$a2 8($sp)			# save the new storage area
	sw	$a0 4($sp)			# save pointer to old object
	jal	_SncGC_QHeapObj			# do object check
	beq	$a0 $0 _SncGC_ChkCopy_notobj	# if not an object, quit
	sw	$a0 4($sp)			# save pointer to new object
	lw	$a0 8($sp)			# load the new storage area
	jal	_SncGC_QLimits			# get the limits of the new area
	lw	$t0 4($sp)
	lw	$t0 obj_disp($t0)		# check for forwarding pointer
	blt	$t0 $a0 _SncGC_ChkCopy_copy
	blt	$t0 $a1 _SncGC_ChkCopy_moved
_SncGC_ChkCopy_copy:
	addiu	$t0 $0 -1			# put eyecatcher before object
	sw	$t0 0($gp)
	addiu	$gp $gp 4
	move	$t0 $gp				# save pointer to new object
	lw	$t1 4($sp)			# load source pointer
	lw	$t2 obj_size($t1)		# load number of words to copy
	sll	$t2 $t2 2			# convert words to bytes
	add	$t2 $t2 $t1			# find the ending address
	bge	$t1 $t2 _SncGC_ChkCopy_done	# check if done
_SncGC_ChkCopy_loop:
	lw	$v0 0($t1)			# load a word from source
	sw	$v0 0($gp)			# save in destination
	addiu	$t1 $t1 4			# update pointers
	addiu	$gp $gp 4
	blt	$t1 $t2 _SncGC_ChkCopy_loop	# finished copying
_SncGC_ChkCopy_done:
	lw	$t1 4($sp)			# get address of old object
	sw	$t0 obj_disp($t1)		# leave forwarding pointer
_SncGC_ChkCopy_moved:
	move	$a0 $t0				# return pointer to new area
	b	_SncGC_ChkCopy_end
_SncGC_ChkCopy_notobj:
	lw	$a0 4($sp)			# restore pointer to old object
_SncGC_ChkCopy_end:
...
# Query Heap Object
#
#   Verify that a pointer actually points to an object in the heap.
#   Three tests are performed:
#     1) The pointer is within the bounds of the specified storage area
#     2) The word before the pointer is the eye catcher 0xFFFF FFFF
#     3) The word at the pointer is a valid tag (i.e. not equal to
#        0xFFFF FFFF)
#
#   INPUT:
#	$a0: pointer to check
#	$a1: storage area to verify object
#	$s7: limit pointer of this area of storage
#	heap_start: start of the heap
#
#   OUTPUT:
#	$a0: set to 0 if not an object, otherwise, not modified
#
#   Registers modified:
#	$t0,$t1,$a0,$a1
#

	.globl	_SncGC_QHeapObj
_SncGC_QHeapObj:
	addi	$sp $sp -8
	sw	$ra 8($sp)			# save the return address
	sw	$a0 4($sp)			# save the pointer to test
	move	$a0 $a1				# set the area to check
	jal	_SncGC_QLimits			# get the limits of storage
	lw	$t0 4($sp)			# load $t0 with pointer to test
	blt	$t0 $a0 _SncGC_QHeapObj_no	# check pointer in storage bounds
	bge	$t0 $a1 _SncGC_QHeapObj_no
	move	$a0 $t0				# load pointer to test in $a0
	addiu	$t0 $0 -1			# set $t0 to 0xFFFF FFFF
	lw	$t1 obj_eyecatch($a0)		# check for the eye catcher
	bne	$t0 $t1 _SncGC_QHeapObj_no	# eye catcher mismatch
	lw	$t1 obj_tag($a0)		# check for a valid tag
	bne	$t0 $t1 _SncGC_QHeapObj_end	# tag is valid
_SncGC_QHeapObj_no:
...
#
# Query current storage area
#
#   Gets the current storage area.  The $s7 register is used
#   to determine which area is active be comparing it to the
#   middle limit pointer in SncGC header.
#
#   INPUT:
#	$s7: limit pointer of this area of storage
#	heap_start: start of the heap
...
_SncGC_QArea:
	la	$t0 heap_start			# set $t0 to the start of the heap
	lw	$t0 4($t0)			# set $t1 to the middle pointer
	bne	$s7 $t0 _SncGC_QArea_area1	# find out active storage area
_SncGC_QArea_area0:
	move	$a0 $0				# set area number to 0
	jr	$ra				# return
_SncGC_QArea_area1:
	addiu	$a0 $0 1			# set area number to 1
	jr	$ra				# return

#
# Query area storage limits
#
#   The lower limit of storage area #n is always the word #n
#   in SncGC header.  The upper limit is word #(n+1).
#
#   INPUT:
#	$a0: area number
#	$s7: limit pointer of this area of storage
...
#
#   OUTPUT:
#	$a0: lower limit of storage
#	$a1: upper limit of storage
#
#   Registers modified:
#	$t0
#

	.globl	_SncGC_QLimits
_SncGC_QLimits:
	sll	$a0 $a0 2
	la	$t0 heap_start			# set $t0 to the start of the heap
	add	$t0 $t0 $a0			# find the lower limit
	lw	$a0 0($t0)			# load the limits into $a0, $a1
	lw	$a1 4($t0)
	jr	$ra				# return
...
#
	...
	.globl	_SncGC_SetRegMask
_SncGC_SetRegMask:
	li	$t0 SncGC_ARU_MASK		# apply Automatic Register Mask (ARU)
	and	$a0 $a0 $t0
	la	$t0 heap_start			# set $t0 to the start of the heap
	sw	$a0 16($t0)			# save the Register mask
	jr	$ra				# return
...
#
	...
	.globl	_SncGC_QRegMask
_SncGC_QRegMask:
	la	$a0 heap_start			# set $a0 to the start of the heap
	lw	$a0 16($a0)			# get the Register mask
	jr	$ra				# return
